...
export function LatestContent() {
  return (
    <section id="contenu" className="bg-white px-5 py-24 sm:px-8 lg:px-10 lg:py-36">
        ...
        >
          {contentCards.map(({ platform, title, description, metric, href, Icon }) => (
            <motion.article
              ...
              variants={fadeUp}
              transition={{ duration: 0.65, ease: "easeOut" }}
              className="group flex min-h-[23rem] flex-col rounded-[1.5rem] border border-stone-200 bg-[#fbfaf8] p-7 shadow-xl shadow-stone-950/[0.035] transition duration-300 hover:-translate-y-1 hover:border-[#b78a54]/35 hover:bg-white hover:shadow-2xl hover:shadow-stone-950/[0.07] sm:p-8"
            >
              <div className="flex items-center justify-between gap-4">
                <span className="inline-flex items-center gap-2 rounded-full border border-stone-200 bg-white px-4 py-2 text-sm font-semibold text-stone-800">
                  <Icon className="h-4 w-4 text-[#9b7444]" strokeWidth={1.8} />
                  {platform}
                </span>
                <span className="text-sm font-semibold text-stone-500">{metric}</span>
              </div>
